...
emailEmitter.on("sendEmail", async (email)=>{
    //login email
    const emailPayload = { email: email.toLowerCase() };

    const token = jwt.sign(
      emailPayload,
      "SecretEasyClinc",
      { expiresIn: 60 * 60 * 24 * 365 } // سنة
    );
    
    const refreshToken = jwt.sign(
      emailPayload,
      "SecretEasyClinc",
      { expiresIn: 60 * 60 * 24 } // يوم
    );
    
    // روابط عامة بدون دور
    const baseUrl = process.env.BASE_URL || 'https://dash-board-abg.vercel.app/';
